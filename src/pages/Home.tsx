...
const [user,setUser] = useState<User>()
  ...
  useEffect(() => {
    ...
    axios
      .get(BASE_URL_OFERTA + "/GetCotizacionesOferta", config)
      .then((res) => {
  ...
  const downloadPDF = (transactionId:number) =>
  {
    ...
    const [i,setI] = useState(0)
          ...
          setI(user?.avatar != undefined ? parseInt(user.avatar) : 0)
  return (
      <Grid
    textAlign="justify"
    mt={5}
      ...
      <Card key={key} color="#ffffff" bg="#003459" m={10} maxW="md">
        <CardHeader>
            <Flex flex="1" gap="4" alignItems="center" flexWrap="wrap">
              <Avatar name={userName} src={maleAvatar[i]} size="md" />
              <Box>
...
const [cotizaciones,setCotizaciones] = useState<CotizacionTotal[]>([])
  ...
  useEffect(() => {
    ...
    axios
      .get(BASE_URL_TOTAL + "/GetCotizacionesTotal", config)
      .then((res) => {
        setCotizaciones(res.data);
        // console.log(res.data)
